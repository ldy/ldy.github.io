...
官网下载解压找到`nexus.exe`程序，安装启动后默认访问地址为`http://localhost:8081/`。
...
nexus.exe /help    查看有哪些命令

# etc目录下有个nexus-default.properties配置文件，里面包含nexus服务的端口和ip地址
# Linux版本不包含install、uninstall命令，且需要单独安装JDK，Windows版本内置了Jre，如果需要自启动需要配置Systemd服务。
# 从Windows迁移到Linux，需要安装相同版本，然后拷贝sonatype-work目录即可，该目录可跨平台迁移，包含了仓库配置、用户权限、构件文件等。
```

Linux版本通过`vim /etc/systemd/system/nexus.service`新增配置文件，并通过`systemctl enable --now nexus`启动并设置开机自启动。

```ini
[Unit]
Description=Nexus Repository Manager 3.70.1-02
...
Restart=on-failure
RestartSec=10
Environment=JAVA_HOME=/home/nexus/jdk1.8.0_202
ExecStart=/home/nexus/nexus-3.70.1-02/bin/nexus start
ExecStop=/home/nexus/nexus-3.70.1-02/bin/nexus stop
