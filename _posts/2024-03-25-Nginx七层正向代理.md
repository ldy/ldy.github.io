...
在机器B配置好正向代理转发配置如下，机器A正向代理转发配置了`connect_module`如上节所示 。
...
在另外一台机器，通过设置系统代理，或者`curl http://www.baidu.com -v -x 10.122.10.218:8080`访问正常，`curl https://www.baidu.com -v -x 10.122.10.218:8080`访问不正常。如果需要访问`https`协议，可以通过发送`http`请求，机器A强制正向代理到`https`协议。

> Client Machine C --> Nginx Machine B --> Nginx Machine A --> target website

另一种方式是前面机器B配置好正向代理请求`http`，机器A再配置正向代理将`http`请求强制转为`https`请求，都无需安装`ngx_http_proxy_connect_module`模块，发送请求的时候通过`http`协议。
